...
def run_script():
    if(len(n_run_entry.get())>0 and len(kernel_entry.get())>0):# and len(target_entry.get())>0):
        ...
        if(len(file_path)>0):
            message_label.config(text="Algoritmo in esecuzione....007")
            window.update()
            start_time = time.time()
            ...
            for i in range(run):
                ...
                if(i==run-1):
                    ...
                    print(f"Tempo di esecuzione dell'algoritmo AAAAAAAAAAAAAAAAAAAA: {execution_time} secondi.")
                else:
