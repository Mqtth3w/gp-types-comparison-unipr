...
def modularGP(run, max_depth, generations, iterations, individual_to_keep, file_path, kernel_size, pop_size, const):
    ...
    def evalTestSet(individual):
        ...
        f1_t, test_pred, _, _ = training_RF(new_train_set, train_labels, new_test_set, test_labels)
    ...
    # Registriamo nuovi operatori di mutazione e crossover
    ...
    # Limiti di altezza per operatori
...
Introduzione di nuovi operatori genetici:

mate_subtree: Un crossover di sottoalbero che sostituisce l'intero sottoalbero di un genitore con quello dell'altro.
mutate_subtree: Una mutazione di sottoalbero che sostituisce l'intero sottoalbero con un nuovo sottoalbero generato casualmente.
mutate_point: Una mutazione puntuale che modifica un singolo nodo.
mutate_insert: Una mutazione che inserisce un nuovo nodo.
Operatori genetici adattivi:

adjust_probabilities: Una funzione che adatta dinamicamente le probabilità di mutazione e crossover basate sulla diversità della popolazione e sul miglioramento della fitness.
Implementando queste modifiche, dovresti vedere un miglioramento nei valori di F1 grazie alla maggiore diversità genetica e all'adattamento dinamico delle probabilità di mutazione e crossover.
